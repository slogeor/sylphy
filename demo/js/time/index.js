...
var option = {
  nationCode: 'ch',
  startDate: undefined,
  endDate: new Date(date.getTime() + 24 * 3600 * 1000),
  supportDay: 1,
...
//倒计时结束
function endCB(timeObj) {
  $('#cutdown-time').html('end');
};

// 倒计时继续
function continueCB(timeObj) {
  var timeArr = [];
  for (var key in timeObj) {
    timeArr.push(timeObj[key]);
  }
  $('#cutdown-time').html(timeArr.join(' : '));
};

time.getCountDown(option);


//分钟倒计时
var minOption = {
  continueCB: function(second) {
    $('#min-time').html(second + 's');
  },
  endCB: function() {
    $('#min-time').html('倒计时结束!');
  }
...
$('#min-time').on('click', function() {
  time.minTimer(minOption);
});